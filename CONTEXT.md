...
**VIG**:
A special badge tier at Gen Con granting access to VIG-only Events. The acronym's meaning is not known to this codebase — treat as an opaque Gen Con concept.
...
**ChangeLog**:
A batch record of Events created, updated, or deleted in a single data pull from Gen Con. Pulls are scheduled externally in Railway (approximately 3× per day). Each ChangeLog entry is the authoritative record of what changed and when.
...
- **VIG**: What does the acronym stand for? Is it a badge tier, a membership program, or something else?
- **SpecialCategory**: What do "Gen Con Presents" and "Premier Event" mean to attendees in practice? What distinguishes the two?
- **ChangeLog pull cadence**: Approximately 3× per day was estimated, but the exact schedule is in Railway and was not confirmed.
