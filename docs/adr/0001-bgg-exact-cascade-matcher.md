...
The `bgg_mapping.json` file is committed to the repo so every `data update` run is reproducible against the same mapping until someone explicitly reruns `data match-bgg`.